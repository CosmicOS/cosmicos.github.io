...
/* A MISSING ARC FILE IS A FAILURE, NOT AN EMPTY ONE. This read used to fall back to '' — so a
   keeper whose file had not been generated passed every check below in silence: no premature tokens,
   no orphan marks, nothing to report, because there was nothing to read. ORDER is a hand-kept list
   (scripts/arc-order.js) and this directory is GENERATED by prose.js, so the two can come apart
   without anyone touching this file, and green would have meant "an arc is gone" as readily as
   "the arc is clean". */
for (const f of ORDER) {
  const p = path.join(dir, f + '.html');
  if (!fs.existsSync(p)) {
    console.error(`✗ ${p} is missing — it is in scripts/arc-order.js but not on disk.`);
    console.error(`  Run scripts/prose.js build (it writes ${dir}/ from _prose/), or drop the name from arc-order.`);
    process.exit(2);
  }
